class SortingRobot:
    ...
    def sort(self):
        ...
        while self.light_is_on():
            ...
            # once we reach left where all items are sorted and we're holding largest item, swap
                if self.compare_item() == None:
                    self.swap_item()
                    break
            # check if we can move to right and move right
            while self.can_move_right():
                self.move_right()
                # if at last item and nothing else to move turn light off.
                if self.can_move_right() == False and self.compare_item() == None:
                    self.set_light_off()
                    break
                # compare items with holding and if return is 1 swap
